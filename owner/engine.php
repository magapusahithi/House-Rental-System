...
function add_property(){
	...
	global $property_id,$country,$province,$district,$city,$vdc_municipality,$ward_no,$tole,$contact_no,$property_type,$estimated_price,$total_rooms,$bedroom,$living_room,$bathroom,$description,$latitude,$path,$p_photo,$property_photo_id,$longitude,$owner_id,$booked,$db;
	...
	$latitude=validate($_POST['latitude']);
        ...
        if(mysqli_num_rows($result1)>0)
      {
   	...
   	$sql = "INSERT INTO add_property(country,province,district,city,vdc_municipality,ward_no,tole,contact_no,property_type,estimated_price,total_rooms,bedroom,living_room,bathroom,description,latitude,longitude,booked,owner_id) VALUES('$country','$province','$district','$city','$vdc_municipality','$ward_no','$tole','$contact_no','$property_type','$estimated_price','$total_rooms','$bedroom','$living_room','$bathroom','$description','$latitude','$longitude','$booked','$owner_id')";
...
function owner_update(){
	...
	$password = md5($password); // Encrypt password
		$sql = "UPDATE owner SET full_name='$full_name',email='$email',phone_no='$phone_no',address='$address'";
		$query=mysqli_query($db,$sql);
		if(!empty($query)){
			?>
